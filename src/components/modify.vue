<template>
    <div class="login">
      <div class="login-header"><span  @click="goHistroy('/systemset')"><img src="../../static/img/left.png" alt=""></span><i></i></div>
      <h1>密码修改</h1>
      <div class="form">
        <p>
        ...
        <p>
          ...
          <p>
          <input :type="flag"  id="code1"  v-model="login.pwd" placeholder="请输入6-16位密码">
          <i @click="showYj"><img :src="fimg" alt=""></i>
        </p>
      </div>
      <div class="login-button" @click="Resg()"><img src="../../static/img/success.png" alt=""></div>
      ...
      methods:{
          ...
          showYj(){
            if(this.flag=='password'){
               this.flag='text'
               this.fimg='../../static/img/yanjing.png'
            }else{
              this.flag='password'
              this.fimg='../../static/img/yanF.png'
            }
          },
          formSet:function(){
          ...
          Resg(){
            ...
            var _this=this;
            var json={phone:this.login.phone,password:this.login.pwd,phone_code:this.login.code};
            console.log(json)
           // var param= this.$root.entrypt(json);
            this.axios.post(api.wantpwd,this.$qs.stringify(json))
            .then(function(res){
              if(res.data.state==0){
                  _this.showM(res.data.msg+'1秒后跳转');
                  setTimeout(function(){
                    _this.goHistroy('/login_pwd')
                  },1000)
                }else{
                  _this.showM(res.data.msg);
                }
