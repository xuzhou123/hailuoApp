<template>
    ...
    <div class="page-infinite-wrapper"   ref="wrapper" :style="{ height: wrapperHeight + 'px',overflow:'scroll' }" >
            <ul id="live-list"  class="page-infinite-list"  v-infinite-scroll="loadMore" infinite-scroll-disabled="loading" infinite-scroll-distance="50">
              <li  :style="styleObj(item.pull+'?vframe/jpg/offset/1')" v-for="(item,index) in live" @click="LinkRouter(item.pull)" >
                	<div class="fanavatar" >
                		 <div class="h-info">
		                 <span class="fans">{{item.content}}</span>
		                  <span class="live-icon"><i class="iconp">&#xe729;</i>{{item.scan_count}}</span>
		                </div>
