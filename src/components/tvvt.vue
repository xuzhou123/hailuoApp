<template>
  <div ref="wrapper" class="hot">
    <header>热度</header>
    <div
      class="page-infinite-wrapper"
      ref="wrapper"
      :style="{ height: wrapperHeight + 'px',overflow:'scroll' }"
    >
      <ul
        id="live-list"
      ...
      >
        <li
          :style="styleObj(item.pull+'?vframe/jpg/offset/1')"
          v-for="(item,index) in live"
          @click="LinkRouter(item.pull)"
        >
          <div class="fanavatar">
            <div class="h-info">
              <span class="fans">{{item.content}}</span>
...
export default {
  ...
  methods: {
    ...
    getLive(o) {
      ...
      this.axios
        .post(api.hot_small_video, this.$qs.stringify(json))
        .then(function(res) {
          if (res.data.state == 0) {
    ...
    getNice(o) {
      var _this = this;
      this.axios.get(api.h5).then(function(res) {
        if (res.data.state == 0) {
        }
      });
    },
