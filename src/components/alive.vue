	...
	<div id="live">
		<div class="wrapper" ref="wrapper">
			...
			<div class="video"  >
					...
					<div class="tanmu_scroll" id="tanmuScroll"  @click="videoflag()">
						<ul>
                            ...
                            <li v-for="(item,index) in live"    class="normal">
                                ...
                                <span  @click="arrkuty({'id':item.uid})" v-if="item.msgtype=='1'" class="nick"><img style="width:40px;"  v-if="item.guizu!=''"  :src="guizu2(item.guizu)"  alt=""><img v-if="item.liangname!='0'" style="width:20px;" src="../../static/img/y7.png" alt="">&nbsp;<img :src="imgct(item)" alt="">&nbsp;{{item.uname}} <i style="color:#fff;">送出</i> {{item.ct.giftname}}x{{item.ct.giftcount}}</span>
                ...
                <div class="video_status video_status_start"><img src="../../static/img/video_start.png" alt=""> {{timerstart}}</div>-
				<img style="display:none;" :src="liveCt.avatar" onload="_yy_firstScreenTime = Date.now();">
				<div class="top_banner" @click="arrkuty({'id':videoUrl})">
					<img class="avatar"  style="object-fit: cover;"  :src="liveCt.avatar==1?require('../../static/img/default.jpg'):liveCt.avatar" :alt="liveCt.user_nicename" >
					<div class="text text_status_living">
						<p class="nick">{{liveCt.user_nicename}}</p>
						<p class="count">
				...
				<div class="chat_banner" v-show="!valueinput" v-if="!flag">
					...
					<i class="closegif chat_show_pop_gift js-download" @click="closewindow()"></i>
					<!--<span class="chat_btn js-download" ><span class="huya_icon"></span>打开App参与精彩互动 <i class="icon_arrow"></i></span>-->
				</div>
				<div class="input"  v-show="valueinput">
					<input type="text"  @keypress="CheckInfo" v-model="valueInfo" placeholder="说点什么吧！"><button @click="Send">发送</button>
				</div>
				...
				<div :class="['Banner3',{'active':java2}]" style="width:220px;height:40px; position: fixed;bottom:295px;left:0;">
				...
				<div :class="['Banner3',{'active':java3}]" style="width:220px;height:40px; position: fixed;bottom:240px;left:0;">
					<div class="Banner2" data-id="undefined" @click="videoflag()">
		...
		<div :class="['numbercss3',{'avtivecdd3':facebook}]" v-if="facebook">{{textnumber}}</div>
		...
		<div class="share_model" v-if="pkflag" @click="pkFun1()"></div>
		<div class="pkflag_info"  v-if="pkflag1">
				<p>互动PK</p>
			<div class="model">
				<span @click="pkFun2()" >1</span>
				<span>2</span>
				<span>3</span>
			</div>

		</div>
		<div class="pkflag_info"  v-if="pkflag2">
			<p>约战PK-礼物比拼</p>
			<div class="mdiv">
				<div class="model1">
					<div class="span" v-for="(item,index) in pklist" @click="pleasePK(item.uid)">
						<span ><img :src="item.avatar" alt=""></span>
						<i>{{item.user_nicename}}</i>
					</div>
				</div>
			</div>
		</div>
...
<script>
    ...
    export default {
        ...
        sockets:{
            broadcastingListen:function(o){
                ...
                for(var i in o){
                    ...
                    } else if (msgmethod == 'testlink') { //连麦pk
                        _this.testlink(data.msg[0]);
        ...
        methods:{
			...
			pleasePK(e){
                var pktime=5
                var msg = '{"retcode":"000000","retmsg":"ok","msg":[{"_method_":"testlink","action":"1","msgtype":"10","roomnum":"' + e + '","user_nicename":"' + this.liveCt.user_nicename + '","quid":"' + this.videoUrl + '","buid":"' + e + '","pktime":"' + pktime + '"}]}';
                this.clickButton(msg)
			},
            pkFun3(quid,buid){//请求一次
                var _this=this;
                var json={quid:quid,buid:buid};
                this.axios.post(apiy.live_link_show_live,this.$qs.stringify(json))
                    .then(function(res){
                        var dat=res.data
						if(dat.state==0){
						    _this.quid=quid
			...
			closePK(){
                ...
                this.axios.post(apiy.live_unlink_show_live,this.$qs.stringify(json))
                    .then(function(res){
                        var dat=res.data
                        if(dat.state==0){
                            _this.showM(dat.msg)
            ...
            testlink(data){
                ...
                if (action == 1) {
                    ...
                    //更新礼物我方主播的礼物
                    var lwcount = data.lwcount;
                    alert(lwcount+'我放主播礼物')
                } else if (action == 3) {//未知
                    gpkstart()
                    if (roomnum == this.videoUrl) {
                    ...
                    if (roomnum == this.videoUrl) {
                        this.showM("请求失败");
                        var msgtype = data.msgtype;  //pk时间
                        if (msgtype != 9) {
                            clearInterval(Linkmic_interval);
                        }
                    }
                    ...
                    //更新礼物对方主播的礼物
                    var lwcount = data.lwcount;
                    alert(lwcount+'对放主播礼物')
                } else if (action == 11) {
                    //更新礼物我方主播的礼物
                    var lwcount = data.lwcount;
                    alert(lwcount+'更新礼物我方主播的礼物')
                } else if (action == 12) {
                    //pk结果
					alert('pk结果')
                   /* result()
                    if (roomnum == this.videoUrl) {
                        livestate(quid, buid, 1);
                    }*/
                }
            ...
            getpkList(){
                var _this=this;
                this.axios.get(apiy.live_pk_link_list)
                    .then(function(res){
                        var dat=res.data
                        if(dat.state==0){
                            _this.pklist=dat.content
                        }
                    })
            },
            guizu2(e){
            ...
            qqInfo(o){
                ...
                var qq = api.require('QQPlus');
                qq.shareNews({
                    url: 'http://live.hailuozhibo.com/h5/view1/#/'+_this.liveCt.uid+'&code='+localStorage.getItem('code'),
                    title: _this.liveCt.user_nicename+'正在直播！[海螺直播]',
                    description: '好久不见,想念你们，如果你也想看看最近我的改变，欢饮来到我的直播间！',
                    imgUrl:'',
                    type:o
                },function(ret,err){
                    if (ret.status){
            ...
            shareInfo(o){
                ...
                this.wx.shareWebpage({
                    ...
                    thumb: '',
                    contentUrl:apiy.hosturl+'live?id='+_this.liveCt.uid+'&code='+localStorage.getItem('code')
                }, function(ret, err) {
                    if (ret.status) {
                        alert('分享成功');
                    } else {
            ...
            closewindow(){
                var _this=this;
                this.axios.post(apiy.live_close_show_live_room)
                    .then(function(res){
                        //if(dat.state==0){
                            _this.closegif()
                        //}
                ...
                this.axios.get(apiy.setNodeInfo+'?liveuid='+o)
                    .then(function(res){
                        ...
                        if(res.data.state==0){
                            ...
                            console.log(_this.liveCt)
                            _this.$socket.emit('conn', {uid:res.data.userinfo.id,roomnum:res.data.userinfo.roomnum,stream:_this.liveCt.stream,nickname: res.data.userinfo.user_nicename,equipment: 'pc',token:res.data.userinfo.token});
                            //console.log({uid:res.data.userinfo.id,roomnum: _this.liveCt.liang.name,stream:_this.liveCt.stream,nickname: res.data.userinfo.user_nicename,equipment: 'pc',token:res.data.userinfo.token})
                            // };
                        }
                    })
            },
                ...
                this.axios.post(apiy.home+'?'+this.$qs.stringify(json))
                    .then(function(res){
                        ...
                        if(dat.state==0){
                            ...
                            _this.catdog=dat.content.show_total_contribution
                            _this.catperson=dat.content.liveinfo.audience_num
                            _this.fity_length=(dat.content.gift_list.length%8)==0?(dat.content.gift_list.length/8):Math.ceil(dat.content.gift_list.length/8)
                            // console.log(_this.fity_length,dat.content.gift_list.length)
                           if(_this.islive===0) _this.arrkuty({'id':_this.videoUrl})
                            /*_this.playvideo()*/
                            //_this.locKcoin=dat.content.user_info.coin
                        }
                        // _this.live=res.data.content.show_list
                        //_this.$refs.html5player.play()
...
<style type="text/css" scoped="scoped" lang="less">
	...
	.avtivecdd3{
		...
		-moz-animation: animat 1s linear;	/* Firefox */
		-webkit-animation: animat 1s linear;	/* Safari 和 Chrome */
		-o-animation: animat 1s linear;
	}
	@keyframes animat{
